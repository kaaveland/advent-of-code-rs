...
fn parse_equations(input: &str) -> anyhow::Result<Vec<Equation>> {
    input.lines()
        ...
        .map(|line| {
            ...
            let operands: anyhow::Result<Vec<_>> = rest.split_whitespace()
                .filter(|n| !n.is_empty())
                .map(|n| Ok(n.parse()?))
                .collect();
            Ok(Equation {
                equals, operands: operands?
            })
        })
        .collect()
}
...
where F: Fn(u64, u64) -> u64 {
    if left > target {
        ...
        ops.iter().any(|op| reduces(op(left, rest[0]), target, &rest[1..], ops))
    }
}

fn sum_reducible<F>(equations: &[Equation], ops: &[F]) -> u64
where F: Fn(u64, u64) -> u64 {
    equations.iter().filter_map(|eq| {
...
pub fn part_2(input: &str) -> anyhow::Result<String> {
    let equations = parse_equations(input)?;
    let n = sum_reducible(&equations, &vec![|x, y| x * y, |x, y| x + y, |x, y| concat(x, y)]);
    Ok(format!("{n}"))
}
